...
function emmwt_frontend_maintenance_redirect() {
    ...
    // SMART UPGRADE: Allow admins to preview the design EVEN IF maintenance mode is currently disabled!
    ...
    // Strip Admin Bar for accurate Preview
    if ( $is_preview ) {
    ...
    if ( ! empty( $date ) ) {
        ...
        if ( $expiry_timestamp && $current_timestamp >= $expiry_timestamp ) {
            // Timer expired, stay at 00:00:00 for preview mode
            $seconds_remaining = 0; 
        } else {
    ...
    <head>
        ...
        <style>
            body.emmwt-maintenance-mode {
                ...
                position: relative; z-index: 1; overflow: hidden; /* Prevent zoom scrollbars */
                background-color: <?php echo esc_attr( $bg_overlay_color ); ?>; /* Fallback */
            }

            .emmwt-bg-layer {
                position: absolute; top: -5%; left: -5%; right: -5%; bottom: -5%;
                z-index: -1;
                <?php if ( ! empty( $bg_url ) && $bg_animation !== 'gradient' ) : ?>
                    background: linear-gradient(<?php echo $rgba; ?>, <?php echo $rgba; ?>), url('<?php echo esc_url( $bg_url ); ?>') no-repeat center center;
            ...
            /* Animation 1: Slow Zoom (Ken Burns) */
            <?php if ( $bg_animation === 'zoom' && ! empty( $bg_url ) ) : ?>
            .emmwt-bg-layer {
                animation: emmwt-zoom 25s ease-in-out infinite alternate;
            }
            @keyframes emmwt-zoom {
                0% { transform: scale(1); }
                100% { transform: scale(1.1); }
            }
            <?php endif; ?>

            /* Animation 2: Smooth Color Gradient */
            <?php if ( $bg_animation === 'gradient' ) : ?>
            .emmwt-bg-layer {
                /* Mixes Overlay Color with Message Color */
                background: linear-gradient(270deg, <?php echo esc_attr( $bg_overlay_color ); ?>, <?php echo esc_attr( $msg_color ); ?>);
                background-size: 400% 400%;
                animation: emmwt-gradient 12s ease infinite;
            }
            @keyframes emmwt-gradient {
    ...
    <body class="emmwt-maintenance-mode" style="<?php echo esc_attr( $body_style ); ?>">
        ...
        <div class="emmwt-content-wrapper">
            ...
            <?php if ( get_option( 'emmwt_enable_subscribe', 0 ) ) : ?>
                <div class="emmwt-subscribe-wrapper" style="margin-top: 30px; margin-bottom: 10px;">
                    <form id="emmwt-subscribe-form" style="display: flex; justify-content: center; max-width: 450px; margin: 0 auto; gap: 8px; flex-wrap: wrap;">
                        ...
                        <button type="submit" id="emmwt-subscribe-btn" 
                                style="padding: 12px 25px; background-color: black; ?>; color: #ffffff; border: none; border-radius: 4px; font-size: 1em; font-weight: bold; cursor: pointer; transition: opacity 0.3s; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    if(form) {
                        form.addEventListener('submit', function(e) {
                            ...
                            var emailInput = document.getElementById('emmwt-subscribe-email');
                            ...
                            .catch(function(error) {
                                ...
                                btn.style.opacity = '1';
                                msgBox.style.display = 'block';
                                msgBox.innerHTML = '<span style="color: #f44336;"><?php esc_html_e( "Network error. Please try again.", "easy-maintenance-timer" ); ?></span>';
                            });
                        });
                    }
                });
